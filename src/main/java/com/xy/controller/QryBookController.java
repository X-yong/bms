 ...
 */
@Controller
@RequestMapping("qryBookController")
public class QryBookController {
	
	@Resource
	private BookService bookService;
	
	@RequestMapping("qryBookInfo")
	public String  qryBookInfo(HttpServletRequest request, BookVo bookVo) {
		try {
			String startDate = request.getParameter(constants.startDate);
			String endDate = request.getParameter(constants.endDate);
			bookVo.setB_startDate(startDate);
			bookVo.setB_endDate(endDate);
